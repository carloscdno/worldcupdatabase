...
echo "$($PSQL "SELECT (SELECT SUM(winner_goals) FROM games) + (SELECT SUM(opponent_goals) FROM games) AS total_sum")"
...
echo "$($PSQL "WITH subquery AS (SELECT GREATEST(winner_goals, opponent_goals) AS max_goals FROM games) SELECT MAX(max_goals) AS max_overall FROM subquery;")"
